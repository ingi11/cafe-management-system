...
<div class="min-h-screen flex items-center justify-center -mt-12" style="background: linear-gradient(120deg, #fef9e8 0%, #fff5ea 100%);">
    <div class="w-full max-w-md bg-white rounded-[2.5rem] shadow-[0_20px_50px_rgba(45,17,2,0.05)] p-12 border border-amber-100/20">
        ...
        <div class="text-center mb-10">
            <span class="inline-block p-4 bg-amber-50 rounded-2xl mb-4 text-2xl">✏️</span>
            <h2 class="text-3xl font-black text-[#2d1102] uppercase tracking-tighter">Edit Category</h2>
            <p class="text-amber-900/40 text-[11px] font-bold uppercase tracking-widest mt-1">Update your menu structure</p>
        </div>
        ...
        <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            ...
            {{-- Image Upload --}}
            <div>
                <label class="block text-[10px] font-black text-amber-900/40 uppercase tracking-[0.2em] mb-3 ml-1">Change Image (Optional)</label>
